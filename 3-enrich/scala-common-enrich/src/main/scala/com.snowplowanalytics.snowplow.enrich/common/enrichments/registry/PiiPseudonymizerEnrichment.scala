...
import com.snowplowanalytics.iglu.client.validation._
 ...
 * @param schemaCriterion the schema for which the startegy will be applied
          ...
          case ja: JArray => {
            ja.transform {
              case JObject(List(("schema", JString(schema)), ("data", jobject)))
                  if (SchemaKey.parse(schema).map(schemaCriterion.matches(_)).getOrElse(false)) => {
                JObject(List(("schema", JString(schema)), ("data", jsonPathReplace(jobject))))
              }
            }
          }
        })
